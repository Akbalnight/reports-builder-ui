...
const Item = ({icon, title, contentClassName, isLoading, children, contentRef}) => {
    ...
    return (
        <div className="rbu-builder-item">
            <div className="rbu-builder-item-title">
                {icon && <Icon style={{fontSize: '16px'}} type={icon} />}
                <span>{title}</span>
                {actions && <div className="rbu-builder-item-actions">{actions}</div>}
            </div>
            <Spin spinning={isLoading || false}>
                <div className="rbu-builder-item-content">
                    <div ref={contentRef} className={contentClassName}>{content}</div>
                    {footer && <div className="rbu-builder-item-footer">{footer}</div>}
                </div>
            </Spin>
        </div>
    )
